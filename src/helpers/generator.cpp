...
void generate_players(entt::registry& registry)
{
    ...
    std::for_each(
        ...
        [&registry](const entt::entity& player) {
            registry.patch<component::name>(player, [](component::name& name) {
                name._first_name = vp::helper::ng::instance().get_name();
                name._last_name = vp::helper::ng::instance().get_surname();
            ...
            registry.patch<component::nickname>(
                player, [&registry, &player](component::nickname& nickname) {
